import type { Metadata } from "next";
import Image from "next/image";
import Link from "next/link";

export const metadata: Metadata = {
  title: "Raske nettsider — ytelse og Lighthouse-scorer | Dev AS",
  description:
    "Nettsider som laster på under ett sekund. Vi bygger med Next.js og statisk generering, og optimaliserer for Core Web Vitals fra dag én.",
};

const techniques = [
  {
    id: "01",
    title: "Statisk generering",
    body: "Sider genereres ved build-tid og serveres direkte fra CDN. Ingen databasekall per request, ingen server-render-delay. Bare filer.",
  },
  {
    id: "02",
    title: "Bildoptimalisering",
    body: "Automatisk WebP-konvertering, lazy loading og riktige størrelser for hver skjermoppløsning. Bilder er ofte den største ytelsessynderen — vi fikser det.",
  },
  {
    id: "03",
    title: "Minimal JavaScript",
    body: "Vi sender bare det som faktisk brukes på siden. Ingen tunge rammeverk lastet for funksjonalitet siden ikke trenger.",
  },
  {
    id: "04",
    title: "Font-optimalisering",
    body: "Fonter lastes med font-display: swap og preloades korrekt. Ingen layout-shifts fordi fonter ankommer for sent.",
  },
  {
    id: "05",
    title: "CDN-distribusjon",
    body: "Innhold serveres fra noder nær brukeren. En besøkende i Tromsø venter ikke på en server i Frankfurt.",
  },
  {
    id: "06",
    title: "HTTP caching",
    body: "Riktige cache-headere sørger for at nettleseren ikke laster ned det samme to ganger. Gjentakende besøk er nesten øyeblikkelige.",
  },
];

const metrics = [
  {
    label: "LCP",
    full: "Largest Contentful Paint",
    target: "Under 2,5 sek.",
    desc: "Når lastes det største elementet i viewport? Dette er det viktigste ytelsessignalet for Google.",
  },
  {
    label: "INP",
    full: "Interaction to Next Paint",
    target: "Under 200 ms",
    desc: "Hvor raskt reagerer siden på input? Treg interaktivitet frustrerer brukere og svekker rangeringen.",
  },
  {
    label: "CLS",
    full: "Cumulative Layout Shift",
    target: "Under 0,1",
    desc: "Hopper innholdet rundt mens siden laster? Det er en dårlig brukeropplevelse — og Google straffer det.",
  },
];

const cornerBrackets = [
  "top-6 left-6 border-l-2 border-t-2",
  "top-6 right-6 border-r-2 border-t-2",
  "bottom-6 left-6 border-l-2 border-b-2",
  "bottom-6 right-6 border-r-2 border-b-2",
];

export default function RaskeNettsiederPage() {
  return (
    <div className="flex min-h-screen flex-col bg-white">
      <header className="border-b border-slate-100">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-5">
          <Link
            href="/"
            className="flex items-center gap-2 text-sm font-light text-slate-500 transition-colors hover:text-slate-900"
          >
            ← Forsiden
          </Link>
          <Link href="/" className="flex items-center gap-2">
            <Image
              src="/logo-icon.svg"
              alt=""
              width={16}
              height={18}
              aria-hidden="true"
            />
            <span className="font-mono text-xs text-slate-400">Dev AS</span>
          </Link>
        </div>
      </header>

      <main className="flex-1">
        {/* Hero */}
        <section className="relative overflow-hidden border-b border-slate-100 px-6 py-24 sm:py-32">
          <div
            className="pointer-events-none absolute inset-0 opacity-[0.3]"
            style={{
              backgroundImage:
                "radial-gradient(circle, #94a3b8 1px, transparent 1px)",
              backgroundSize: "28px 28px",
            }}
            aria-hidden="true"
          />
          <div
            className="pointer-events-none absolute top-0 right-0 h-[600px] w-[600px] opacity-20"
            style={{
              background:
                "radial-gradient(ellipse at top right, #a7ea00 0%, transparent 65%)",
            }}
            aria-hidden="true"
          />
          <div className="relative z-10 mx-auto max-w-3xl">
            <p className="mb-4 text-xs font-medium tracking-[0.2em] text-slate-400 uppercase">
              Dev AS — ytelse
            </p>
            <h1 className="text-4xl font-light leading-tight tracking-tight text-slate-900 sm:text-5xl lg:text-6xl">
              Raske{" "}
              <span className="font-semibold">nettsider</span>
            </h1>
            <p className="mt-3 text-2xl font-light tracking-tight text-slate-400 sm:text-3xl">
              Ytelse er ikke en funksjon. Det er fundamentet.
            </p>
            <p className="mt-8 max-w-xl text-lg leading-relaxed font-light text-slate-500">
              En nettside som laster tregt mister besøkende, rangerer dårligere
              og gir et dårlig inntrykk. Vi bygger sider som er raske fra start
              — ikke optimaliserte i etterkant.
            </p>
          </div>
        </section>

        {/* Why speed matters */}
        <section className="border-b border-slate-100 px-6 py-20">
          <div className="mx-auto max-w-3xl">
            <p className="mb-4 text-xs font-medium tracking-[0.2em] text-slate-400 uppercase">
              Hvorfor det betyr noe
            </p>
            <h2 className="mb-8 text-2xl font-light tracking-tight text-slate-900 sm:text-3xl">
              Hvert sekund teller —{" "}
              <span className="font-semibold">bokstavelig talt</span>
            </h2>
            <div className="space-y-5 text-sm leading-relaxed font-light text-slate-600">
              <p>
                Studier fra Google viser at sannsynligheten for at en bruker
                forlater siden øker med 32 prosent når lastetiden går fra ett
                til tre sekunder. Det er ikke et tall å overse.
              </p>
              <p>
                I tillegg er ytelse et direkte rangeringssignal. Core Web Vitals
                er en del av Googles Page Experience-signal, og sider med dårlig
                ytelse konkurrerer på bortebane mot raskere alternativer.
              </p>
              <p>
                For bedrifter som selger tjenester eller produkter på nett er
                konverteringseffekten enda mer direkte. En raskere side er
                en mer lønnsom side.
              </p>
            </div>
          </div>
        </section>

        {/* Techniques */}
        <section className="border-b border-slate-100 bg-slate-950 px-6 py-20">
          <div className="mx-auto max-w-5xl">
            <div className="mb-12">
              <p className="mb-3 text-xs font-medium tracking-[0.2em] text-slate-500 uppercase">
                Hva vi gjør
              </p>
              <h2 className="text-2xl font-light tracking-tight text-white sm:text-3xl">
                Seks teknikker vi bruker —{" "}
                <span className="font-semibold">som standard</span>
              </h2>
            </div>

            <div className="grid gap-px bg-slate-800 sm:grid-cols-2 lg:grid-cols-3">
              {techniques.map(({ id, title, body }) => (
                <div key={id} className="flex flex-col bg-slate-950 p-8">
                  <span
                    className="mb-5 inline-block self-start px-2 py-0.5 font-mono text-xs font-medium"
                    style={{ background: "#a7ea00", color: "#0f172a" }}
                  >
                    {id}
                  </span>
                  <h3 className="mb-3 text-base font-semibold tracking-tight text-white">
                    {title}
                  </h3>
                  <p className="text-sm leading-relaxed font-light text-slate-400">
                    {body}
                  </p>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* Core Web Vitals explained */}
        <section className="border-b border-slate-100 bg-slate-50 px-6 py-20">
          <div className="mx-auto max-w-5xl">
            <div className="mb-12">
              <p className="mb-3 text-xs font-medium tracking-[0.2em] text-slate-400 uppercase">
                Googles målinger
              </p>
              <h2 className="text-2xl font-light tracking-tight text-slate-900 sm:text-3xl">
                Core Web Vitals —{" "}
                <span className="font-semibold">hva de faktisk måler</span>
              </h2>
            </div>

            <div className="space-y-px bg-slate-200">
              {metrics.map(({ label, full, target, desc }) => (
                <div
                  key={label}
                  className="flex items-start gap-8 bg-white px-8 py-7"
                >
                  <div className="shrink-0">
                    <span
                      className="inline-block px-2 py-0.5 font-mono text-xs font-medium"
                      style={{ background: "#a7ea00", color: "#0f172a" }}
                    >
                      {label}
                    </span>
                  </div>
                  <div>
                    <div className="mb-1.5 flex flex-wrap items-baseline gap-3">
                      <h3 className="text-sm font-semibold tracking-tight text-slate-900">
                        {full}
                      </h3>
                      <span className="font-mono text-xs text-slate-400">
                        mål: {target}
                      </span>
                    </div>
                    <p className="text-sm font-light leading-relaxed text-slate-500">
                      {desc}
                    </p>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* CTA */}
        <section className="relative overflow-hidden bg-slate-950">
          <div
            className="pointer-events-none absolute inset-0"
            style={{
              backgroundImage: `
                linear-gradient(rgba(255,255,255,0.04) 1px, transparent 1px),
                linear-gradient(90deg, rgba(255,255,255,0.04) 1px, transparent 1px)
              `,
              backgroundSize: "48px 48px",
            }}
            aria-hidden="true"
          />
          {cornerBrackets.map((cls) => (
            <div
              key={cls}
              className={`pointer-events-none absolute h-8 w-8 border-brand opacity-20 ${cls}`}
              aria-hidden="true"
            />
          ))}
          <div className="relative z-10 mx-auto max-w-2xl px-6 py-24 text-center">
            <p className="mb-4 text-xs font-medium tracking-[0.2em] text-slate-500 uppercase">
              Treg nettside?
            </p>
            <h2 className="text-3xl font-light tracking-tight text-white sm:text-4xl">
              Vi vet hva som bremser den
            </h2>
            <p className="mx-auto mt-4 max-w-xl font-light text-slate-400">
              Send oss en lenke til nettsiden din, og vi gir deg en konkret
              vurdering av hva som kan forbedres.
            </p>
            <div className="mt-8 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
              <Link
                href="/#kontakt"
                className="group inline-flex items-center gap-2 bg-brand px-8 py-3.5 text-sm font-medium tracking-wide text-slate-950 transition-all hover:bg-white"
              >
                Ta kontakt
                <span className="transition-transform duration-200 group-hover:translate-x-0.5">
                  →
                </span>
              </Link>
              <Link
                href="/teknisk-seo"
                className="text-sm font-light text-slate-400 transition-colors hover:text-slate-300"
              >
                Les om teknisk SEO →
              </Link>
            </div>
          </div>
        </section>
      </main>

      <footer className="border-t border-slate-800 bg-slate-950">
        <div className="mx-auto max-w-5xl px-6 py-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-xs font-light text-slate-600">
              Dev AS © 2018–{new Date().getFullYear()}
            </p>
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
              <Link
                href="/ressurser"
                className="text-xs font-light text-slate-600 transition-colors hover:text-slate-400"
              >
                Ressurser
              </Link>
              <Link
                href="/filosofi"
                className="text-xs font-light text-slate-600 transition-colors hover:text-slate-400"
              >
                Filosofi
              </Link>
              <Link
                href="/personvern"
                className="text-xs font-light text-slate-600 transition-colors hover:text-slate-400"
              >
                Personvern
              </Link>
              <Link
                href="/"
                className="text-xs font-light text-slate-600 transition-colors hover:text-slate-400"
              >
                Forsiden
              </Link>
            </div>
          </div>
        </div>
      </footer>
    </div>
  );
}
