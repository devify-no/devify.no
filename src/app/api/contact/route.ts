...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: resendError } = await resend.emails.send({
      ...
      text: `Ny henvendelse fra nettside\n\nNavn: ${name}\nE-post: ${email}\nMelding: ${message}\n\nreCAPTCHA Score: ${score}`,
      html: `<h2>Ny henvendelse fra nettside</h2><p><strong>Navn:</strong> ${name}</p><p><strong>E-post:</strong> ${email}</p><p><strong>Melding:</strong></p><p>${message.replace(/\n/g, "<br>")}</p><hr><p><small>reCAPTCHA Score: ${score}</small></p>`,
    });
